(function(GLOBAL, $, tmpl) {
  ...
  function _newTag(e) {
	...
	$.parse.get("pieces", {where : { objectId: id }}, function(json) {
		...
		console.log("get title: ", json.results[0].title);
		...
		if (tagExists < 0) { // if piece does NOT have this tag
			...
			$.parse.get("tags", {where : {"tagName": val}}, function(json) {
				...
				if (results.length == 0) {
					...
					//tag = $(tmpl("tag_tmpl", { objectId: false, tagName: val, pieceIds: [id] }));
					//tag_list.append(tag);
					//console.log("added to tag_list");
					
					$.parse.post('tags', { tagName: val, pieceIds: [id] }, function(json) { 
						//tag.data('id', json.objectId);
						console.log("new tagid ", json.objectId);
						console.log("new tagid ", json.createdAt);
						
						// add tagId to tagCounts
						tagCounts[val][1] = json.objectId;
						console.log("tag[1]", tagCounts[val][1]);
						$.parse.put("pieces/" + id, { tagCounts: tagCounts }, function(json) {
							console.log('updated piece tags ' + id, json)
						});
					});
					...
					oldpieces = json.results[0].pieceIds;
					...
					$.parse.put("tags/" + tagId, { pieceIds: newpieces }, function(json) {
						console.log('updated tag pieces' + id, json)
					});
					
					tagCounts[val][1] = tagId;
					$.parse.put("pieces/" + id, { tagCounts: tagCounts }, function(json) {
						console.log('updated piece tags ' + id, json)
					});
				}
			...
			// piece's tag count ++
			tagCounts[val][0] = tagCounts[val][0] + 1;
		...
		for (var key in tagCounts) {
			newHTML += '<a href="tag.html?i='+tagCounts[key][1]+'">';
			newHTML += '<span class="tag tag-'+tagCounts[key][0]+'">'+key+'</span>';
			newHTML += '</a>';
		}
